...
def load_pnn(checkpoint, num_prim, has_lateral, activation="relu", device="cpu"):
    ...
    mlp_args = {
        "input_size": state_dict_load["a2c_network.pnn.actors.0.0.weight"].shape[1],
        "units": layer_size[:-1],
    ...
    pnn = PNN(
        mlp_args,
        output_size=checkpoint["model"]["a2c_network.mu.bias"].shape[0],
        numCols=num_prim,
...
def load_z_encoder(checkpoint, activation="relu", z_type="sphere", device="cpu"):
    ...
    if (
        ...
        net_key_name = (
            "a2c_network._task_mlp"
            if "a2c_network._task_mlp.0.weight" in checkpoint["model"].keys()
            else "a2c_network.z_mlp"
        )
    elif z_type == "hyper":
...
def load_linear(net_name, checkpoint):
    net = nn.Linear(
        checkpoint["model"][net_name + ".weight"].shape[1],
        checkpoint["model"][net_name + ".weight"].shape[0],
    )
    state_dict = net.state_dict()
...
def load_z_decoder(checkpoint, activation="relu", z_type="sphere", device="cpu"):
    ...
    if z_type == "vq_vae":
        quantizer_weights = checkpoint["model"][
            "a2c_network.quantizer.embedding.weight"
        ]
        quantizer = Quantizer(
            quantizer_weights.shape[0], quantizer_weights.shape[1], beta=0.25
        )
        ...
        z_type == "vae" and "a2c_network.z_prior.0.weight" in checkpoint["model"].keys()
    ):
        prior_loading_keys = [
            k
            for k in checkpoint["model"].keys()
            if k.startswith("a2c_network.z_prior.")
        ]
        ...
        if "a2c_network.z_prior_logvar.weight" in checkpoint["model"].keys():
            z_prior_logvar = load_linear(
                "a2c_network.z_prior_logvar", checkpoint=checkpoint
            )
            z_prior_logvar.eval()
