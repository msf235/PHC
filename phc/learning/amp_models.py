...
class ModelAMPContinuous(ModelA2CContinuousLogStd):
    ...
    class Network(ModelA2CContinuousLogStd.Network):
        ...
        def forward(self, input_dict):
            ...
            if is_train:
                amp_obs, amp_obs_replay, amp_demo_obs = (
                    input_dict["amp_obs"],
                    input_dict["amp_obs_replay"],
                    input_dict["amp_obs_demo"],
                )
        ...
        def get_dropout_mask(self, amp_obs, steps, num_masks=3, dropout_rate=0.3):
            ...
            for idx_joint in range(num_joints):
                ...
                dropout_mask[
                    :,
                    dof_joints_offset + idx_joint * 6 : dof_joints_offset
                    + idx_joint * 6
                    + 6,
                    :,
                ] = has_drop_out[:, None]
                dropout_mask[
                    :,
                    dof_vel_offsets + idx_joint * 3 : dof_vel_offsets
                    + idx_joint * 3
                    + 3,
                    :,
                ] = has_drop_out[:, None]
