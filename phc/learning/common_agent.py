...
class CommonAgent(a2c_continuous.A2CAgent):
    def __init__(self, base_name, config):
        ...
        if self.normalize_input:
            if "vec_env" in self.__dict__:
                obs_shape = torch_ext.shape_whc_to_cwh(
                    self.vec_env.env.task.get_running_mean_size()
                )
            else:
    ...
    def _assemble_train_info(self, train_info, frame):
        train_info_dict = {
            ...
            "learning_rate/last_lr": train_info["last_lr"][-1]
            * train_info["lr_mul"][-1],
        ...
        if "actor_loss" in train_info:
            train_info_dict.update(
                {
                    "loss/actor_loss": torch_ext.mean_list(
                        train_info["actor_loss"]
                    ).item(),
                    "loss/critic_loss": torch_ext.mean_list(
                        train_info["critic_loss"]
                    ).item(),
                    "loss/bounds_loss": torch_ext.mean_list(
                        train_info["b_loss"]
                    ).item(),
                    "loss/entropy": torch_ext.mean_list(train_info["entropy"]).item(),
                    "loss/clip_frac": torch_ext.mean_list(
