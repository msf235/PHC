...
class HumanoidTennisServe(humanoid_amp_task.HumanoidAMPTask):
    def __init__(
        ...
        self._contact_sensor_body_ids = self._build_key_body_ids_tensor(
            cfg.env.contact_sensor_bodies
        )
        ...
        self.bounding_box_batch = self.bounding_box.unsqueeze(0).repeat(
            self.num_envs, 1, 1
        )
        ...
        (
            self.obj_names,
            self.env_to_obj_name,
            self.obj_name_to_code,
            self.obj_asset_root,
        ) = [], {}, {}, {}
        for data_key in list(data_seq.keys()):
            obj_name = (
                data_seq[data_key]["obj_data"]["obj_info"][0]
                .split("/")[-1]
                .split(".")[0]
            )
            self.obj_names.append(obj_name)
            self.obj_name_to_code[obj_name] = data_seq[data_key]["obj_data"][
                "object_code"
            ]

            if (
                "oakink" in data_seq[data_key]["obj_data"]["obj_info"][0]
                or "omomo" in data_seq[data_key]["obj_data"]["obj_info"][0]
            ):
                self.obj_asset_root[obj_name] = "/".join(
                    data_seq[data_key]["obj_data"]["obj_info"][0].split("/")[:-1]
                )
                self.has_data = False
            else:
                self.obj_asset_root[obj_name] = "phc/data/assets/urdf/grab/"

        self.obj_names_current_envs = list(self.obj_names)[
            : self.num_envs
        ]  # only load the same number of objects as the number of envs.
        self.obj_names_unique = sorted(
            list(set(self.obj_names))
        )  # all unique object names
        # self.obj_names_unique = list(self.obj_names) # all unique object names

        if cfg.env.get("tile_env", False):
            self.obj_names_current_envs = [
                self.obj_names_unique[i % len(self.obj_names_unique)]
                for i in range(self.num_envs)
            ]

        self.obj_names_unique_load = sorted(
            list(set(self.obj_names_current_envs))
        )  # unique object names for loading

        self.obj_ids_unique_load = torch.tensor(
            [self.obj_names_unique.index(name) for name in self.obj_names_unique_load]
        )
        self.data_seq_to_obj_id = torch.tensor(
            [self.obj_names_unique.index(name) for name in self.obj_names]
        )
        self.env_to_obj_name = [
            self.obj_names_current_envs[i % len(self.obj_names_current_envs)]
            for i in range(self.num_envs)
        ]  #
        self.env_to_obj_id = torch.tensor(
            [self.obj_names_unique.index(name) for name in self.env_to_obj_name]
        )

        if self.cfg.env.get("add_noise", False):
            noise_scale = self.cfg.env.get("noise_scale", 0.01)
            self.env_to_obj_code = torch.cat(
                [
                    self.obj_name_to_code[self.env_to_obj_name[i]]
                    + (2 * torch.rand_like(self.obj_name_to_code[obj_name]) - 1)
                    * noise_scale
                    for i in range(self.num_envs)
                ],
                dim=0,
        ...
        self.table_remove_frame = torch.zeros(self.num_envs).to(
            self.device
        )  # 45 frames, 1.5 second
        ...
        print(
            {
                k: v
                for k, v in np.asarray(
                    np.unique(self.env_to_obj_name, return_counts=True)
                ).T
            }
        )
        ...
        self._obj_contact_forces = self.contact_force_tensor.view(
            self.num_envs, self.bodies_per_env, 3
        )[..., -2:, :]  # gater both table and object contact forces.

        self._cycle_counter = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.int
        )
        self.eval_time_coutner = torch.zeros(
            self.num_envs, device=self.device, dtype=torch.int
        )

        self._hand_body_ids = self._build_key_body_ids_tensor(cfg.env.hand_bodies)
        self._fingertip_body_ids = self._build_key_body_ids_tensor(
            cfg.env.fingertip_bodies
        )
        self._hand_pos_prev = torch.zeros(
            (self.num_envs, len(self._hand_body_ids), 3)
        ).to(self.device)
        self._hand_dof_ids = [
            [
                self._dof_names.index(hand_name) * 3,
                self._dof_names.index(hand_name) * 3 + 1,
                self._dof_names.index(hand_name) * 3 + 2,
            ]
            for hand_name in self.cfg.env.hand_bodies
        ]
        self._hand_dof_ids = torch.from_numpy(np.concatenate(self._hand_dof_ids)).to(
            self.device
        )
        self.last_contacts = torch.zeros(
            self.num_envs,
            len(self._contact_body_ids),
            dtype=torch.bool,
            device=self.device,
            requires_grad=False,
        )
    ...
    def _build_traj_generator(self):
        ...
        if self.cfg.env.get("use_orig_traj", False):
            ...
            self._traj_gen = traj_generator_3d_orig.TrajGenerator3D(
                num_envs,
                episode_dur,
                num_verts,
                self.device,
                self.cfg.env.traj_gen,
                starting_still_dt=self.table_remove_frame * self.dt,
            )
        else:
            self._traj_gen = traj_generator_3d.TrajGenerator3D(
                num_envs,
                episode_dur,
                num_verts,
                self.device,
        ...
        self._traj_gen.reset(
            env_ids,
            init_pos=self._obj_states[:, 0:3],
            init_rot=self._obj_states[:, 3:7],
            time_till_table_removal=self.table_remove_frame.max() * self.dt,
        )
    ...
    def _create_smpl_humanoid_xml(self, num_humanoids, smpl_robot, queue, pid):
        ...
        for idx in num_humanoids:
            ...
            if not smpl_robot is None:
                ...
                asset_file_real = (
                    f"phc/data/assets/mjcf/smpl_{int(gender_beta[0])}_humanoid.xml"
                )
    ...
    def recreate_sim(self, failed_keys, epoch=0):
        if self.cfg.env.obj_pmcp and epoch >= self.cfg.env.get("obj_pmcp_start", 999):
            ...
            if epoch // self.cfg.learning.params.config.eval_frequency % 2 == 0:
                print("Extreme::::::")
                tau = 500
                per_obj_fail_rate = self._motion_lib.compute_per_obj_fail_rate(
                    failed_keys
                )
                obj_fail_array = np.array(
                    [per_obj_fail_rate[n] for n in self.obj_names_unique]
                )
                ...
                obj_multiplier = 1
                if (
                    len(self.obj_names) < 200
                    and self.num_envs > len(self.obj_names) * 10
                ):
                    obj_multiplier = 10

                num_available_envs = (
                    self.num_envs - len(self.obj_names) * obj_multiplier
                )
                new_env_to_obj_ids = np.random.choice(
                    np.arange(len(self.obj_names_unique)),
                    num_available_envs,
                    p=all_prob,
                    replace=True,
                )
                new_env_to_obj_ids = np.sort(new_env_to_obj_ids)
                self.env_to_obj_name = self.obj_names * obj_multiplier + [
                    self.obj_names_unique[i] for i in new_env_to_obj_ids
                ]  #
            else:
                print("Random::::::")
                obj_multiplier = 1
                ...
                new_env_to_obj_ids = np.array(
                    [i % len(self.obj_names_unique) for i in range(num_available_envs)]
                )
                new_env_to_obj_ids = np.sort(new_env_to_obj_ids)
                self.env_to_obj_name = self.obj_names * obj_multiplier + [
                    self.obj_names_unique[i] for i in new_env_to_obj_ids
            ...
            if num_available_envs > 0:
                if self.cfg.env.get("add_noise", False):
                    noise_scale = self.cfg.env.get("noise_scale", 0.01)
                    self.env_to_obj_id[-num_available_envs:] = torch.from_numpy(
                        new_env_to_obj_ids
                    )  # this will also update motion lib's copy
                    self.env_to_obj_code[:] = torch.cat(
                        [
                            self.obj_name_to_code[self.env_to_obj_name[i]]
                            + (
                    ...
                    self.env_to_obj_code[-num_available_envs:] = torch.cat(
                        [
                            self.obj_name_to_code[
                                self.obj_names_unique[new_env_to_obj_ids[i]]
                            ]
                            for i in range(num_available_envs)
                        ],
                        dim=0,
                    ).to(self.device)  # this will also update network's copy
    ...
    def _load_motion(self, motion_train_file, motion_test_file=[]):
        ...
        if self.humanoid_type in ["smpl", "smplh", "smplx"]:
            ...
            motion_lib_cfg = EasyDict(
                {
                    ...
                    "obj_rand_pos_extreme": self.cfg.env.get(
            ...
            raise Exception(
                "Unsupported humanoid type: {:s}".format(self.humanoid_type)
            )
    ...
    def get_task_obs_size(self):
        ...
        if self._enable_task_obs:
            if self.cfg.env.obs_v in [1]:
                obs_size = (
                    15 * self._num_traj_samples + 9 + 10 * 3
                )  # local pos and orientation 10 ; 9 is object position and orientation in root.  ; 10 * 3 for finger to object root position.
    ...
    def _load_target_asset(self):
        ...
        for obj_name in self.obj_names_unique_load:
            ...
            if "oakink" in obj_asset_root:
                asset_options.vhacd_params.resolution = (
                    512  # Oakink object will trigger a werid bug and segfault.
                )
            ...
            _target_asset = self.gym.load_asset(
                self.sim, obj_asset_root, asset_file, asset_options
            )
        ...
        _target_asset = self.gym.load_asset(
            self.sim, "phc/data/assets/urdf/grab/", asset_file, asset_options_fix
        )
    ...
    def _build_target(self, env_id, env_ptr):
        ...
        target_handle = self.gym.create_actor(
            env_ptr,
            self._target_assets[self.env_to_obj_name[env_id]],
            default_pose,
            "target",
            env_id,
            0,
        ...
        self.gym.set_rigid_body_color(
            env_ptr, target_handle, 0, gymapi.MESH_VISUAL, color_vec
        )

        target_handle = self.gym.create_actor(
            env_ptr, self._target_assets["table"], default_pose, "target", env_id, 0
        )  # Table
        self._target_handles.append(target_handle)
        return
    ...
    def _build_target_tensors(self):
        ...
        self._obj_states = self._root_states.view(
            self.num_envs, num_actors, self._root_states.shape[-1]
        )[..., 1, :]
        self._table_states = self._root_states.view(
            self.num_envs, num_actors, self._root_states.shape[-1]
        )[..., 2, :]

        self._all_obj_ids = torch.stack(
            [
                to_torch(
    ...
    def _reset_target(self, env_ids):
        ...
        motion_res = self._get_state_from_motionlib_cache(
            self._sampled_motion_ids[env_ids], self._motion_start_times[env_ids]
        )

        ref_o_ang_vel, ref_o_lin_vel, ref_o_rb_rot, ref_o_rb_pos = (
            motion_res["o_ang_vel"][:, :1],
            motion_res["o_lin_vel"][:, :1],
            motion_res["o_rb_rot"][:, :1],
            motion_res["o_rb_pos"][:, :1],
        )
        ...
        static_o_ang_vel, static_o_lin_vel, static_o_rb_rot, static_o_rb_pos = (
            motion_res["o_ang_vel"][:, 1:],
            motion_res["o_lin_vel"][:, 1:],
            motion_res["o_rb_rot"][:, 1:],
            motion_res["o_rb_pos"][:, 1:],
        ...
        self._table_states[env_ids, 2] -= (
            0.1 - 0.005481
        ) / 2  # This is becuase I am using a thicker table.
        ...
        self._traj_gen.reset(
            env_ids,
            init_pos=self._obj_states[env_ids, 0:3],
            init_rot=self._obj_states[env_ids, 3:7],
            time_till_table_removal=self.table_remove_frame[env_ids].max() * self.dt,
        )
    ...
    def _sample_ref_state(self, env_ids):
        ...
        if (
            ...
            if self.cfg.env.get("flex_start", True):
                ...
                motion_times = torch.clamp(
                    motion_times,
                    torch.zeros_like(motion_times),
                    self._motion_lib.get_contact_time(self._sampled_motion_ids[env_ids])
                    - 0.5,
                )  # Start before contact happens.

                if flags.test:
                    motion_times[:] = 0
            elif self.cfg.env.get("contact_start", False):
                motion_times = self._motion_lib.get_contact_time(
        ...
        if self.cfg.env.get("use_stage_reward", False):
            self.table_remove_frame[env_ids] = (
                (
                    self._motion_lib.get_contact_time(self._sampled_motion_ids[env_ids])
                    - motion_times
                )
                / self.dt
            )  # update table remove frame based on the sample start time of the motion. 0.25 is the raise time of the object. Bascially, the object will be raised within 0.25 second, and the table should be safe to be removed.
        ...
        motion_times[:] = (
            0  # Even though you sampled at a certain time, the internal timer should start at 0
        )
        return (
            self._sampled_motion_ids[env_ids],
            motion_times,
    ...
    def _compute_task_obs(self, env_ids=None):
        ...
        if self._fut_tracks:
            ...
            motion_ids_steps = self._sampled_motion_ids[env_ids].repeat_interleave(
                time_steps
            )
            ...
            ) * self.dt + self._motion_start_times[env_ids]
            motion_ids_steps = self._sampled_motion_ids[env_ids]
            time_steps = 1

        if flags.im_eval and self.has_data:
            motion_res = self._get_state_from_motionlib_cache(
                motion_ids_steps, motion_times_steps
            )
        else:
            motion_res = self._traj_gen.get_motion_state(
                motion_ids_steps, motion_times_steps
            )
        ...
        if self.cfg.env.get("contact_obs", False):
            ...
            contact_obs = compute_contact_force_obs(
                root_pos, root_rot, contact_forces_fingers, self._has_upright_start
            )
        ...
        if self.cfg.env.get("use_hand_flag", False):
            use_hand_label = self._motion_lib.get_use_hand_label(
                self._sampled_motion_ids[env_ids]
            )
    ...
    def get_running_mean_size(self):
        if self.cfg.env.fixed_latent:
            return (
                self.get_obs_size() - 1,
            )  # need multi object index to not be clipped
        else:
    ...
    def _compute_reward(self, actions):
        ...
        contact_filter = check_contact(
            ...
            obj_lin_vel,
            table_removed,
            self.close_distance_contact,
        )

        grab_reward, grab_reward_raw = compute_grab_reward(
            root_pos,
            root_rot,
            obj_pos,
            obj_rot,
        ...
        if self.cfg.env.get("pregrasp_reward", True):
            contact_hand_dict = self._motion_lib.get_contact_hand_pose(
                self._sampled_motion_ids
            )
            (
                ref_contact_hand_pos,
                ref_contact_hand_rot,
                ref_contact_hand_vel,
                ref_contact_hand_ang_vel,
                contact_ref_obj_pos,
            ) = (
                contact_hand_dict["contact_hand_trans"],
                contact_hand_dict["contact_hand_rot"],
                contact_hand_dict["contact_hand_vel"],
                contact_hand_dict["contact_hand_ang_vel"],
                contact_hand_dict["contact_ref_obj_pos"],
            )
            pregrasp_reward, pregrasp_reward_raw = compute_pregrasp_reward_time(
                root_pos,
                root_rot,
                hand_pos,
                hand_rot,
        ...
        self.rew_buf[:], self.reward_raw = (
            grab_reward,
            torch.cat([grab_reward_raw], dim=-1),
        )

        if self.cfg.env.get("penality_slippage", False):
            slipage = torch.clamp(
                self._penality_slippage()
                * self.cfg.env.get("slippage_coefficient", 0.3),
                0,
                1,
            )
        ...
        if self.power_reward:
            power = torch.abs(torch.multiply(self.dof_force_tensor, self._dof_vel)).sum(
                dim=-1
            )
            ...
            self.reward_raw = torch.cat(
                [self.reward_raw, power_reward[:, None]], dim=-1
            )

        if self.cfg.env.get("reward_airtime", False):
            airtime_reward = torch.clamp(
                self._reward_feet_air_time()
                * self.cfg.env.get("airtime_coefficient", 0.5),
                0,
                1,
            )
            self.rew_buf[:] = self.rew_buf + airtime_reward
            self.reward_raw = torch.cat(
                [self.reward_raw, airtime_reward[:, None]], dim=-1
            )
    ...
    def _compute_reset(self):
        ...
        grab_reset, grab_terminate = compute_humanoid_grab_reset(
            self.reset_buf,
            self.progress_buf,
            ...
            self.grab_termination_disatnce,
            flags.no_collision_check,
            self.check_rot_reset and (not flags.im_eval),
        )
        if flags.im_eval:
            if self.has_data:
                motion_res = self._get_state_from_motionlib_cache(
                    self._sampled_motion_ids, torch.zeros_like(time)
                )
            else:
                motion_res = self._traj_gen.get_motion_state(
                    self._sampled_motion_ids, torch.zeros_like(time)
            ...
            obj_hand_has_contact = (self._obj_contact_forces[:, 0].abs() > 1).sum(
                dim=-1
            ) + (self._contact_forces[:, self._hand_body_ids].abs() > 1).sum(
                dim=-1
            ).sum(dim=-1) > 0
            lifted = torch.logical_and(
                ((obj_pos - ref_o_rb_pos_init)[..., 2] > 0.03)[:, 0],
                obj_hand_has_contact,
            )
            ...
            self.success_lift = torch.logical_or(
                self.eval_time_coutner > (0.5 / self.dt), self.success_lift
            )

        self.reset_buf[:], self._terminate_buf[:] = (
            torch.logical_or(self.reset_buf, grab_reset),
            torch.logical_or(self._terminate_buf, grab_terminate),
        )
    ...
    def _reset_ref_state_init(self, env_ids):
        self._cycle_counter[env_ids] = (
            0  # always give 60 frames for it to pick up and catch up.
        )
    ...
    def post_physics_step(self):
        ...
        self.remove_table(
            env_ids=self.all_env_ids[self.progress_buf > self.table_remove_frame]
        )
        ...
        if flags.im_eval:
            motion_times = (
                (self.progress_buf) * self.dt + self._motion_start_times
            )  # already has time + 1, so don't need to + 1 to get the target for "this frame"

            if self.has_data:
                motion_res = self._get_state_from_motionlib_cache(
                    self._sampled_motion_ids, motion_times, None
    ...
    def _build_termination_heights(self):
        ...
        if type(termination_distance) is list:
            ...
            self._termination_distances = to_torch(
                np.array(termination_distance), device=self.device
            )
        else:
            self._termination_distances = to_torch(
        ...
        self.grab_termination_disatnce = to_torch(
            np.array(self.cfg["env"].get("grab_termination_distance", 0.5)),
            device=self.device,
        )
        return
    ...
    def _draw_task(self):
        ...
        eye_offset = torch.tensor(
            [self.eye_offset] * self.num_envs, device=head_rot.device
        )
        pos_near = head_pos[:, 0] + torch_utils.my_quat_rotate(
            head_rot[:, 0], eye_offset
        )

        eyesigt_direction = torch.tensor(
            [[0.0, 0, 0.5]] * self.num_envs, device=head_rot.device
        )
        target = pos_near + torch_utils.my_quat_rotate(
            head_rot[:, 0], eyesigt_direction
        )
        ...
        if not flags.real_traj:
            for env_id in range(self.num_envs):
                line = (
                    torch.cat(
                        [pos_near[env_id].unsqueeze(0), target[env_id].unsqueeze(0)],
                        dim=0,
                    )
                    .cpu()
                    .numpy()
                ...
                self.gym.add_lines(
                    self.viewer, self.envs[env_id], 1, line, (1.0, 0, 0.0)
                )

                sphere_geom_marker_pos_near = gymutil.WireframeSphereGeometry(
                    0.025, 10, 10, None, color=(1, 0, 0.0)
                )
                sphere_pose_pos_near = gymapi.Transform(
                    gymapi.Vec3(
                        pos_near[env_id, 0], pos_near[env_id, 1], pos_near[env_id, 2]
                    ),
                    r=None,
                )
                gymutil.draw_lines(
                    sphere_geom_marker_pos_near,
                    self.gym,
                    self.viewer,
                    self.envs[env_id],
                    sphere_pose_pos_near,
                )

                sphere_geom_marker_target = gymutil.WireframeSphereGeometry(
                    0.025, 10, 10, None, color=(0, 1, 0.0)
                )
                sphere_pose_target = gymapi.Transform(
                    gymapi.Vec3(
                        target[env_id, 0], target[env_id, 1], target[env_id, 2]
                    ),
                    r=None,
                )
                gymutil.draw_lines(
                    sphere_geom_marker_target,
                    self.gym,
                    self.viewer,
                    self.envs[env_id],
                    sphere_pose_target,
        ...
        if flags.show_traj:
            ...
            contact_hand_trans = self._motion_lib.get_contact_hand_pose(
                self._sampled_motion_ids
            )["contact_hand_trans"]
            ...
            motion_times_steps = (
                ...
                + time_internals
                + self._motion_start_times[env_ids, None]
            ).flatten()  # Future poses
            env_ids_steps = self._sampled_motion_ids[env_ids].repeat_interleave(
                time_steps
            )

            if flags.im_eval and self.has_data:
                motion_res = self._get_state_from_motionlib_cache(
                    env_ids_steps, motion_times_steps
                )  # pass in the env_ids such that the motion is in synced.
            else:
                motion_res = self._traj_gen.get_motion_state(
                    env_ids_steps, motion_times_steps
            ...
            o_rb_pos = (
                motion_res["o_rb_pos"].cpu().numpy().reshape(B, time_steps, -1, 3)
            )
                ...
                if flags.real_traj:
                    for time_step in range(time_steps):
                        ...
                        sphere_geom_marker = gymutil.WireframeSphereGeometry(
                            0.025, 10, 10, None, color=(0.0, 1, 0.0)
                        )
                        sphere_pose = gymapi.Transform(
                            gymapi.Vec3(
                                o_rb_pos[env_id, time_step, 0, 0],
                                o_rb_pos[env_id, time_step, 0, 1],
                                o_rb_pos[env_id, time_step, 0, 2],
                            ),
                            r=None,
                        )
                        gymutil.draw_lines(
                    ...
                    for time_step in range(time_steps):
                        ...
                        if time_step == 0:
                            ...
                            sphere_geom_marker = gymutil.WireframeSphereGeometry(
                                ...
                                color=(
                                    0.0,
                                    1 * ((time_steps - time_step) / time_steps + 0.5),
                                    0.0,
                                ),
                            )
                        sphere_pose = gymapi.Transform(
    ...
    def _penality_slippage(self):
        ...
        return torch.sum(
            ...
            * (
                torch.norm(self._contact_forces[:, self._contact_body_ids, :], dim=-1)
                > 1.0
            ),
            dim=1,
        )
    ...
    def _penality_stumble(self):
        return torch.any(
            torch.norm(self._contact_forces[:, self._contact_body_ids, :2], dim=2)
            > 5 * torch.abs(self._contact_forces[:, self._contact_body_ids, 2]),
            dim=1,
        )
    ...
    def _reward_feet_air_time(self):
        ...
        contact = self._contact_forces[:, self._contact_body_ids, 2] > 1.0
...
class HumanoidOmniGraspZ(HumanoidOmniGrasp):
    ...
    def _compute_reward(self, actions):
        ...
        if self.cfg.env.get("res_hand", False):
            ...
            self.reward_raw = torch.cat(
                [self.reward_raw, action_res_reward[:, None]], dim=-1
            )
...
def compute_grab_observations(
    ...
    heading_inv_rot_expand = (
        heading_inv_rot.unsqueeze(-2).repeat((1, J, 1)).repeat_interleave(time_steps, 0)
    )
    heading_rot_expand = (
    ...
    diff_global_body_pos = ref_obj_pos.view(B, time_steps, J, 3) - obj_pos.view(
        B, 1, J, 3
    )
    ...
    diff_global_body_rot = torch_utils.quat_mul(
        ref_obj_rot.view(B, time_steps, J, 4),
        torch_utils.quat_conjugate(obj_rot[:, None]).repeat_interleave(time_steps, 1),
    )
    ...
    diff_global_ang_vel = ref_o_ang_vel.view(B, time_steps, J, 3) - o_ang_vel.view(
        B, 1, J, 3
    )
    ...
    local_o_body_pos = obj_pos.view(B, 1, J, 3) - root_pos.view(
        B, 1, 1, 3
    )  # preserves the body position
    local_o_body_pos = torch_utils.my_quat_rotate(
        heading_inv_rot.view(-1, 4), local_o_body_pos.view(-1, 3)
    )

    local_o_body_rot = torch_utils.quat_mul(
        heading_inv_rot.view(-1, 4), obj_rot.view(-1, 4)
    )
    ...
    local_finger_to_obj_pos = torch_utils.my_quat_rotate(
        heading_inv_rot_expand.view(-1, 4), diff_global_finger_to_obj_pos.view(-1, 3)
    )
...
@torch.jit.script
def compute_pregrasp_reward_time(
    root_pos,
    root_rot,
    hand_pos,
    hand_rot,
    ...
    close_hand_flag = (
        diff_hand_to_object < close_distance
    )  # This flag decides whether the reference hand should be used in computing the pregrasp reward; is it close?
    ...
    distance_filter = (
        curr_dist.sum(dim=-1) / close_hand_flag.sum(dim=-1) > close_distance
    )  # distance filter computes whether the hand is close to reference hand pose. If not close enough, it will be replaced with the "getting closer" reward.

    closer_to_hand_r = (
        torch.clamp(prev_dist - curr_dist, min=0, max=1 / 10).sum(dim=-1)
        / close_hand_flag.sum(dim=-1)
    )  # cap max at 1/10, encourage the hand that is close enough in referect to get closer to the reference
...
def check_contact(
    ...
    vel_filter = torch.logical_or(
        vel_filter, object_lifted
    )  # velocity means the object is moved. The object lifted is for when the object is mid-air and stationary.
    vel_filter[table_removed] = (
        True  # if table is removed, we do not need the proxy check anymore.
    )
...
def compute_grab_reward(
    ...
    w_pos, w_rot, w_vel, w_ang_vel, w_conctact = (
        rwd_specs["w_pos"],
        rwd_specs["w_rot"],
        rwd_specs["w_vel"],
        rwd_specs["w_ang_vel"],
        rwd_specs["w_conctact"],
    )
    ...
    diff_global_body_rot = torch_utils.quat_mul(
        ref_obj_rot, torch_utils.quat_conjugate(obj_rot)
    )
    diff_global_body_angle = torch_utils.quat_to_angle_axis(diff_global_body_rot)[0]
    ...
    reward = (
        ...
        + w_vel * r_lin_vel
        + w_ang_vel * r_ang_vel
    ) * contact_filter + r_contact_lifted * w_conctact
...
@torch.jit.script
def compute_humanoid_grab_reset(
    reset_buf,
    progress_buf,
    contact_buf,
    contact_body_ids,
    ...
    if enable_early_termination:
        has_fallen = torch.any(
            torch.norm(obj_pos - ref_obj_pos, dim=-1) > termination_distance, dim=-1
        )
        ...
        if check_rot_reset:
            ...
            diff_global_body_angle_dist = (diff_global_body_angle**2).mean(dim=-1)
            has_fallen = torch.logical_or(
                has_fallen, diff_global_body_angle_dist > 1
...
def compute_contact_force_obs(root_pos, root_rot, contact_forces_subset, upright):
    ...
    contact_force_local = torch_utils.my_quat_rotate(
        heading_inv_rot_expand.view(-1, 4), contact_forces_subset.view(-1, 3)
    ).view(B, -1)
