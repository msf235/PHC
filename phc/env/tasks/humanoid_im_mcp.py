...
class HumanoidImMCP(humanoid_im.HumanoidIm):
    def __init__(
        ...
        if self.has_pnn:
            ...
            self.pnn = load_pnn(
                ...
                activation=self.z_activation,
                device=self.device,
            )
            self.running_mean, self.running_var = (
                pnn_ck["running_mean_std"]["running_mean"],
                pnn_ck["running_mean_std"]["running_var"],
            )

        if self.mlp_bypass:
            self.mlp_model = MLP(
                input_dim=self.num_obs,
                output_dim=self.num_dof,
                units=[2048, 1024, 512],
                activation="silu",
            )
    ...
    def step(self, weights):
        ...
        with torch.no_grad():
            # Apply trained Model.
            curr_obs = (
                self.obs_buf - self.running_mean.float().to(self.device)
            ) / torch.sqrt(self.running_var.float().to(self.device) + 1e-05)
            ...
            if self.discrete_mcp:
                ...
                weights = torch.nn.functional.one_hot(
                    max_idx, num_classes=self.num_prim
                ).float()
